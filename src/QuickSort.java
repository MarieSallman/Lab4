...
public class QuickSort {
    ...
    static void runFullExperiment(String resultsFileName){
        ...
        for(int inputSize=MININPUTSIZE;inputSize<=MAXINPUTSIZE; inputSize*=2) {
            ...
            for (long trial = 0; trial < numberOfTrials; trial++) {
                ...
                quickSort(testingList, 0, testingList.length-1);
    ...
    public static void quickSort(int[] array, int startIndex, int endIndex) {
        if (startIndex < endIndex) {
            int pivotIndex = ranPivot(array,startIndex,endIndex);
            quickSort(array,startIndex,pivotIndex - 1);
            quickSort(array,pivotIndex + 1,endIndex);
        }
    }

    public static int ranPivot(int[] array, int startIndex, int endIndex) {
        int randomEndIndex = randomNumber(startIndex,endIndex);
